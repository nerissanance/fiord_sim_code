...
simdata_list <-foreach(j=1:R)%do%{
  ...
  train <- as.amtrix(dt[train_index==FALSE])
  test <- as.matrix(dt[train_index==TRUE])
  ...
  for(out in 1:(length(names(dt))-5)){
    ...
    glmnet_model <- cv.glmnet(thisformula, alpha=0, family='binomial',data=train)

    mses <- matrix(nrow=length(cutoffs))

    foreach(j=1:length(cutoffs))%do%{
      pred_j <- as.numeric(predict(glmnet_model,newdata=simdata,type='response')>cutoffs[j])
